...
std::vector<int> fixUpdate(std::vector<int>, std::vector<pageRule>);
...
int solvePuzzle(std::vector<std::string> puzzleInput) {
    ...
    for (int ii = 0; ii < updates.size(); ii++) {
        if (!validateUpdate(updates[ii], rules)) {
            std::cout << "Update " << ii + 1 << " is invalid - fixing it...\n";
            fixedUpdate = fixUpdate(updates[ii], rules);
            
            for (int jj = 0; jj < fixedUpdate.size(); jj++) {
                std::cout << fixedUpdate[jj] << ",";
            }
            std::cout << "\n";

            score += getMiddlePage(fixedUpdate);
...
std::vector<int> fixUpdate(std::vector<int> update, std::vector<pageRule> rules) {
    // declarations
    int currentPage;
    pageRule currentRule;
    bool violationFound = false;
        ...
        for (int jj = 0; jj < rules.size(); jj++) {
            ...
            if (currentRule.firstPage == currentPage) {
                ...
                for (int kk=0; kk<ii; kk++) {
                    if (update[kk] == currentRule.secondPage) {
                        violationFound = true;
                        // fix it: element at kk needs to be moved to the position behind ii
                        toInsert = update[kk];
                        for (int ll=kk; ll<ii; ll++) {
                            update[ll] = update[ll+1];
                        }
                        update[ii] = toInsert;
                        ii = 0;
                        break;
            ...
            if (currentRule.secondPage == currentPage) {
                ...
                for (int kk=ii+1; kk<update.size(); kk++) {
                    if (update[kk] == currentRule.firstPage) {
                        violationFound = true;
                        // fix it: element at kk needs to be moved to the position just before ii
                        toInsert = update[kk];
                        for (int ll=kk; ll>ii; ll--) {
                            update[ll] = update[ll - 1];
                        }
